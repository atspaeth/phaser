...
class FourierSeries(object):
    ...
    def val(self, phi):
        ...
        th = phi * self.om
        return np.exp(1j*th) @ self.coef + self.m
    ...
    def fit(self, order, ph, data):
        """Fit a fourier series to data at phases phi
             ...
             data is a row or two-dimensional array, with data points
             in columns
        """
        ...
        dphi = np.diff(phi)
        self.m = np.sum((dat[:,:-1] + dat[:,1:]) * .5 * dphi[np.newaxis,:],
                        axis=1) / (np.max(phi) - np.min(phi))
        dat = (dat.T - self.m).T
    ...
    def filter(self, coef):
        ...
        if coef.shape == (1,self.coef.shape[1]):
            ...
            if coef.ndim == 1:
                c = np.empty((self.coef.shape[0],1), dtype=self.coef.dtype)
                c[::2,0] = coef
                c[1::2,0] = np.conj(coef)
            elif coef.ndim == 2:
                assert coef.shape[1]==self.coef.shape[1],"Same dimension"
                c = np.empty_like(self.coef)
                c[::2,:] = coef
                c[1::2,:] = np.conj(coef)
            else:
...
class ZScore(object):
    ...
    def __init__(self, y=None, M=None):
        ...
        if M is not None:
            self.fromCovAndMean(np.nanmean(y, 1), M)
        elif y is not None:
            self.fromData(y)
        else:
            self.y0 = None
    ...
    def fromData(self, y):
        ...
        INPUT:
            y -- DxN -- N measurements of a time series in D dimensions
        """
        self.y0 = np.nanmean(y, 1)
        self.M = np.diag(np.nanstd(np.diff(y, n=2, axis=1), axis=1))
...
class Phaser(object):
    ...
    def phaserEval(self, dat):
        ...
        ofs = np.exp(-1j * np.angle(np.nanmean(z0, axis=1)).T)
        ...
        rho = np.nanmean(np.abs(zeta), 1).reshape((zeta.shape[0], 1))
        # compute phase projected onto first principal components
        # using self.prj
        ph = Phaser.angleUp(np.dot(
            self.prj.T, np.vstack([np.cos(p) * rho, np.sin(p) * rho])))

        # return series correction of combined phase using self.P
        phi = np.real(ph + self.P.val(ph).T)
        pOfs2 = (p0[ido0+1] * np.exp(1j * phi.T[ido0+1]) - p0[ido0]
                 * np.exp(1j * phi.T[ido0])) / (p0[ido0+1] - p0[ido0])
    ...
    def phaserTrain(self, y, C=None, ordP=None):
        ...
        if len(set([ele.shape[0] for ele in y])) != 1:
            raise(Exception(
                'newPhaser:dims',
                'All datasets in the ensemble must have the same dimension'))
        D = y[0].shape[0]
        ...
        for k in range(len(y)):
            ...
            if idx.shape[-1] == 0:
                raise Exception('newPhaser:emptySection',
                                'Poincare section is empty -- bailing out')

            svm[:,k] = np.nanmean(sv, 1)
        ...
        for k in range(len(y)):
            ...
            rho_i[k,:] = np.nanmean(np.abs(zetas[k]), 1)
        ...
        # Estimate 10%
        if ordP is None:
            ordP = int(np.floor(np.sum(cycl)/10))
    ...
    def computeOffset(svm, svv):
        ...
        svv = svv / np.sum(svv, 1).reshape(svv.shape[0], 1)

        # compute variance weighted average of phasors on cross
        # section to give the phase offset of each protophase
        mangle = np.sum(svm * svv, 1)
        if any(np.abs(mangle)) < .1:
            b = np.nonzero(np.abs(mangle) < .1)
            raise Exception('computeOffset:badmeasureOfs', len(b)
                            + ' measurement(s), including ' + b[0]
                            + ' are too noisy on Poincare section')

        # compute phase offsets for trials
        mangle = np.conj(mangle) / np.abs(mangle)
        mangle = mangle.reshape((len(mangle), 1))
        svm = mangle * svm
        ofs = np.nanmean(svm, 0)
        if any(np.abs(ofs) < .1):
            b = np.nonzero(np.abs(ofs) < .1)
            raise Exception('computeOffset:badTrialOfs', len(b)
                            + ' trial(s), including ' + b[0]
                            + ' are too noisy on Poincare section')
    ...
    def sliceN(x, s, h=None):
        ...
        INPUT:
            ...
            s -- N, array -- values of function that is zero and
                increasing on surface
            h -- 1x1 (optional) -- threshold for transitions,
                transitions>h are ignored
        OUPUT:
            slc -- DxM -- positions at estimated value of s==0
            idx -- M -- indices into columns of x indicating the last
                point before crossing the surface
        """
        ...
        if x.shape[1] != s.shape[0]:
            raise Exception(
                'sliceN:mismatch',
                'Slice series must have matching columns with data')

        idx = np.flatnonzero((s[1:] > 0) & (s[0:-1] <= 0))
        idx = idx[idx < x.shape[1]]

        if h is not None:
            idx = idx(np.abs(s[idx]) < h & np.abs(s[idx+1]) < h)
    ...
    def seriesCorrection(zetas, ordP):
        ...
        INPUT:
            zetas -- [DxN_1, DxN_2, ...] -- list of D dimensional data
                to be corrected using Fourier series
        ...
        for k in range(len(zetas)):
            ...
            for ki in range(zeta.shape[0]):
                ...
                fdThdt = FourierSeries().fit(
                    ordP * 2, th[ki,:].reshape((1, th.shape[1])),
                    dTheta[ki,:].reshape((1, dTheta.shape[1])) / dt)
                fdThdt.coef = fdThdt.coef / fdThdt.m
                fdThdt.m = np.array([1.])

                # evaluate Fourier series for (d_t/d_theta)(theta)
                # based on Fourier approx of (d_theta/d_t)
                # normalize Fourier coefficients to a mean of 1
                fdtdTh = FourierSeries().fit(
                    ordP, th[ki,:].reshape((1, th.shape[1])),
                    1 / fdThdt.val(th[ki,:].reshape((1, th.shape[1]))).T)
                fdtdTh.coef = fdtdTh.coef / fdtdTh.m
                fdtdTh.m = np.array([1.])

                # evaluate corrected phsae phi(theta) series as
                # symbolic integration of (d_t/d_theta), this is off
                # by a constant. Liberate to this
        ...
        # return phase estimation as weighted average of phase
        # estimation of all samples
        proto_k = []
...
def test_sincos():
    ...
    from pylab import plot, legend, axis, show, randint, randn, rand, lstsq
    ...
    N = 10
    print(N, "trials with:")
    print("\tperiod %.2g" % period,
          "(samples)\n\tSNR %.2g" % snr,
          "\n\tphase noise %.2g" % phaseNoise,"(radian/cycle)")
    print("\tlength = [", end='')
    for li in range(N):
        ltrial = randint(400,2000)  # length of trial
        dt = pi*2.0/period + randn(ltrial)*phaseNoise  # create noisy steps
        t = cumsum(dt)+rand()*2*pi  # starting phase is random
        raw = asarray([sin(t),cos(t)])  # signal
        raw = raw + randn(*raw.shape)/snr  # SNR=20 noise
        t0.append(t)
        dats.append(raw - np.nanmean(raw,axis=1)[:,np.newaxis])
        print(ltrial, end=' ' if li < N-1 else '')
    print("]")
